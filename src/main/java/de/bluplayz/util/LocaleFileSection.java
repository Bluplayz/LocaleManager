...
public class LocaleFileSection extends LinkedHashMap<String, Object> {

    /**
     * Empty LocaleFileSection constructor
     */
    public LocaleFileSection() {
        super();
    }

    /**
     * Constructor of LocaleFileSection that contains initial key/value data
     *
     * @param key
     * @param value
     */
    public LocaleFileSection(String key, Object value ) {
        this();
        this.set( key, value );
    }

    /**
     * Constructor of LocaleFileSection, based on values stored in map.
     *
     * @param map
     */
    public LocaleFileSection(LinkedHashMap<String, Object> map ) {
        this();
        if ( map == null || map.isEmpty() ) return;
        for ( Map.Entry<String, Object> entry : map.entrySet() ) {
            if ( entry.getValue() instanceof LinkedHashMap ) {
                super.put( entry.getKey(), new LocaleFileSection( (LinkedHashMap) entry.getValue() ) );
            } else {
     ...
     * @return
     */
    public LocaleFileSection getAll() {
        return new LocaleFileSection( this );
    }
    ...
    public <T> T get( String key, T defaultValue ) {
        ...
        if ( value != null && value instanceof LocaleFileSection) {
            LocaleFileSection section = (LocaleFileSection) value;
    ...
    public void set( String key, Object value ) {
        ...
        if ( subKeys.length > 1 ) {
            LocaleFileSection childSection = new LocaleFileSection();
            if ( this.containsKey( subKeys[0] ) && super.get( subKeys[0] ) instanceof LocaleFileSection) {
                childSection = (LocaleFileSection) super.get( subKeys[0] );
            }
    ...
    /**
     * Check type of section element defined by key. Return true this element is LocaleFileSection
     *
     * @param key
     * @return
     */
    public boolean isSection( String key ) {
        ...
        return value instanceof LocaleFileSection;
    }
    ...
    public LocaleFileSection getSection(String key ) {
        return this.get( key, new LocaleFileSection() );
    }
     ...
     * a5: 100
     * <p>
     * getSections() will return new LocaleFileSection, that contains sections a1 and a2 only.
     *
     * @return
     */
    //@formatter:on
    ...
    public LocaleFileSection getSections(String key ) {
        LocaleFileSection sections = new LocaleFileSection();
        LocaleFileSection parent = key == null || key.isEmpty() ? this.getAll() : getSection( key );
        if ( parent == null ) return sections;
        parent.entrySet().forEach( e -> {
            if ( e.getValue() instanceof LocaleFileSection) {
    ...
    public void remove( String key ) {
        ...
        if ( super.containsKey( key ) ) {
            ...
            String[] keys = key.split( "\\.", 2 );
            if ( super.get( keys[0] ) instanceof LocaleFileSection) {
                LocaleFileSection section = (LocaleFileSection) super.get( keys[0] );
    ...
    public Set<String> getKeys( boolean child ) {
        ...
        this.entrySet().forEach( entry -> {
            ...
            if ( entry.getValue() instanceof LocaleFileSection) {
                if ( child ) {
                    ( (LocaleFileSection) entry.getValue() ).getKeys( true ).forEach(childKey -> keys.add( entry.getKey() + "." + childKey ) );
                }
            }
        } );
        return keys;
