...
public class LocaleFile {
    ...
    private LocaleFileSection config;
    private java.io.File file;
    private boolean correct;
    private int type;

    public LocaleFile(int type ) {
        this.config = new LocaleFileSection();
    ...
    public LocaleFile(String file ) {
        this( (String) file, -1 );
    }

    public LocaleFile(java.io.File file ) {
        this( (String) file.toString(), -1 );
    }

    public LocaleFile(String file, int type ) {
        this( file, type, new LocaleFileSection() );
    }

    public LocaleFile(java.io.File file, int type ) {
        this( file.toString(), type, new LocaleFileSection() );
    }
     ...
     */
    @Deprecated
    public LocaleFile(String file, int type, LinkedHashMap<String, Object> defaultMap ) {
        this.config = new LocaleFileSection();
        this.nestedCache = new HashMap();
        this.correct = false;
        this.type = -1;
        this.load( file, type, new LocaleFileSection( defaultMap ) );
    }

    public LocaleFile(String file, int type, LocaleFileSection defaultMap ) {
        this.config = new LocaleFileSection();
        this.nestedCache = new HashMap();
        this.correct = false;
        this.type = -1;
     ...
     */
    @Deprecated
    public LocaleFile(java.io.File file, int type, LinkedHashMap<String, Object> defaultMap ) {
        this( file.toString(), type, new LocaleFileSection( defaultMap ) );
    }
    ...
    public void reload() {
        ...
        this.correct = false;
        if ( this.file == null ) {
            throw new IllegalStateException( "Failed to reload Config. LocaleFile object is undefined." );
        } else {
    ...
    public boolean load( String file, int type ) {
        return this.load( file, type, new LocaleFileSection() );
    }

    public boolean load( String file, int type, LocaleFileSection defaultMap ) {
        this.correct = true;
        this.type = type;
        ...
        if ( !this.file.exists() ) {
            try {
                ...
                Logger.getGlobal().log(Level.SEVERE,  "Could not create Config " + this.file.toString() + " | " + e);
            }
    ...
    public boolean save() {
        if ( this.file == null ) {
            throw new IllegalStateException( "Failed to save Config. LocaleFile object is undefined." );
    ...
    public LocaleFileSection getSection(String key ) {
        return this.correct ? this.config.getSection( key ) : new LocaleFileSection();
    }
    ...
    public LocaleFileSection getSections(String key ) {
        return this.correct ? this.config.getSections( key ) : new LocaleFileSection();
    }

    public LocaleFileSection getSections() {
        return this.correct ? this.config.getSections() : new LocaleFileSection();
    }
    ...
    public void setAll( LinkedHashMap<String, Object> map ) {
        this.config = new LocaleFileSection( map );
    }
    ...
    public void setAll( LocaleFileSection section ) {
        this.config = section;
    }

    public LocaleFileSection getRootSection() {
        return this.config;
    }

    public int setDefault( LinkedHashMap<String, Object> map ) {
        return this.setDefault( new LocaleFileSection( map ) );
    }

    public int setDefault( LocaleFileSection map ) {
        int size = this.config.size();
        this.config = this.fillDefaults( map, this.config );
        return this.config.size() - size;
    }

    private LocaleFileSection fillDefaults(LocaleFileSection defaultMap, LocaleFileSection data ) {
        ...
        for ( int var4 = 0; var4 < var3; ++var4 ) {
            ...
            if ( Pattern.compile( "[a-zA-Z0-9\\-_\\.]*+=+[^\\r\\n]*" ).matcher( line ).matches() ) {
                ...
                if ( this.config.containsKey( k ) ) {
                    Logger.getGlobal().log(Level.CONFIG, "[Config] Repeated property " + k + " on file " + this.file.toString());
                }
    ...
    private void parseContent( String content ) {
        switch ( this.type ) {
            ...
            case 1:
                ...
                this.config = new LocaleFileSection( (LinkedHashMap) gson.fromJson( content, (new TypeToken<LinkedHashMap<String, Object>>() {
                } ).getType() ) );
                break;
            case 2:
                DumperOptions dumperOptions = new DumperOptions();
                dumperOptions.setDefaultFlowStyle( FlowStyle.BLOCK );
                Yaml yaml = new Yaml( dumperOptions );
                this.config = new LocaleFileSection( (LinkedHashMap) yaml.loadAs( content, LinkedHashMap.class ) );
                if ( this.config == null ) {
                    this.config = new LocaleFileSection();
                }
                break;
